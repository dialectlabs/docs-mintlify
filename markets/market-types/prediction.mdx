...
<Tip>
  To group markets by event in your application, filter or group the API response
  by the `bundleId` field. This recreates the "event" view from platforms like Kalshi.
</Tip>
...
| `redemptionStatus` | Current redemption state: `null`, `"pending"`, or `"redeemed"` |

<Note>
  The `eventTicker` in `additionalData` corresponds conceptually to the `bundleId`
  field. Both can be used to identify markets that belong together.
</Note>
