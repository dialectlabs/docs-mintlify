...
PnL is only available on **active positions**. Once a user redeems their outcome tokens in full, the position is closed and no longer returned by the [Positions API](/api-reference/positions/list-all-market-positions-by-wallet-address). A dedicated PnL history endpoint will be released in the next days.
...
```json {12-19} Prediction Position with PnL
{
...
- The user holds **1 Yes token** currently worth **\$0.93** (`amountUsd`)
...
- No shares have been sold yet, so:
  ...
  - `unrealized` PnL is **-\$0.02** (current value minus cost basis)
  - `total` is **-\$0.02**
...
The `capital` object is available on each position in the [PnL endpoint](/api-reference/positions/get-pnl-data-for-wallet-positions) and answers the question: **"Have I broken even yet, and how much of my own money is still at stake?"**

Unlike PnL, capital values are **price-agnostic**. They only change when you buy or sell, not when the market moves. This makes the `capital` object a stable measure of your cash commitment to a position.
...
| `capital.netFlow`          | `number` | Net capital flow for the active position: sell proceeds minus buy costs. See below for interpretation. |
| `capital.netFlowLifetime`  | `number` | Net capital flow across all re-entries for this market.                                                |

#### Example Response

```json {11-16} Position with Capital Object (from PnL endpoint)
{
...
`netFlow` is calculated as **sell proceeds − buy costs**. Think of it as the line your P&L cannot cross — it tracks whether you've recouped your original investment through sales:
...
| Zero          | **Breakeven.** You've fully recovered your initial investment through sells.                        |
| Positive      | You've recouped your initial investment and the remaining amount is additional realized returns.    |
...
| Sell for $125 | $200     | $0      | **Breakeven** — initial investment fully recouped   |
| Sell for $50  | $200     | +$50    | $50 above initial investment — pure realized profit |

Note that `invested` only ever increases with buys, regardless of how much you sell. It represents the cumulative cash you've committed, not the current exposure.

### Dedicated PnL Endpoint

Beyond the `pnl` object available on each position, we provide a dedicated [PnL endpoint](/api-reference/positions/get-pn-l-data-for-wallet-positions) that returns aggregated PnL across all a wallet's positions along with the full `capital` breakdown per position.

```typescript Fetching PnL Data
const response = await fetch(
  "https://markets.dial.to/api/v0/positions/pnl?walletAddresses=YOUR_WALLET",
  {
    headers: {
      "x-dialect-api-key": "YOUR_API_KEY",
    },
...
// Top-level aggregates across all positions
console.log(data.total); // Total PnL (realized + unrealized)
console.log(data.realized); // Realized PnL for active positions
console.log(data.realizedLifetime); // Realized PnL including closed positions

// Per-position breakdown including capital
data.positions.forEach((position) => {
  console.log(position.capital.invested); // How much was put in
  console.log(position.capital.netFlow); // Breakeven indicator
});
```

<Tip>
  Use `capital.netFlow` to show users how far they are from breakeven. A value
  of `0` means they've fully recouped their initial investment through sells.
  Anything above `0` is pure realized profit they can treat as "house money."
</Tip>
