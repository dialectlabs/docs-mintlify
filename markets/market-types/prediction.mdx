...
The Positions API returns all positions, including those in settled markets. To get only positions in markets that are still actively trading, filter by `market.status`:
