...
<Note>
  `total` and `totalLifetime` differ when a user has fully exited and re-entered a position. `total` reflects the current open position only, while `totalLifetime` carries forward gains and losses from previous closed episodes.
</Note>
...
<Note>
**Unrealized PnL** is calculated using the **bid-side price** of the outcome token, which represents what a user would actually receive if they sold their position today.
</Note>
