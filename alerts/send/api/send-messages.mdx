...
Add action buttons to your notifications that allow users to take immediate action. Supports up to 3 action buttons that can direct users to external websites or blink-enabled actions. 

When Dialect's [mapping service](/alerts/blinks-in-alerts#mapping-service) detects a supported protocol URL, it automatically enriches the alert with a  `context` object.
...
<Tab title="Blink Action">

The highlighted line shows the supported protocol URL:
...
When a supported URL is detected, the client receives the enriched alert with `context` in an action object:
...
```
<Note>
The `context` object is automatically added by Dialect's mapping service for supported protocols. See [Blinks in Alerts](/alerts/blinks-in-alerts) for complete implementation guide.
</Note>
</Tab>
</Tabs>
...
The `data` parameter allows you to attach arbitrary key-value pairs to your alerts. This data is accessible in notification history and push notification payloads, enabling powerful use cases.
...
curl https://alerts-api.dial.to/v2/{appId}/send \
  ...
  --data '{
    ...
    "message": {
      ...
      "body": "Your swap of 100 USDC to SOL has been completed."
    },
    "data": {
      "type": "transaction",
...
await fetch(`https://alerts-api.dial.to/v2/${appId}/send`, {
  ...
  body: JSON.stringify(alertWithData)
});
```
</Tab>
</Tabs>
...
<Note>
The `data` object is stored with the notification and can be retrieved later from the [notification history endpoint](/api-reference/integrate-an-inbox/get-notification-history). This makes it perfect for implementing deep linking or passing context to your application.
</Note>
