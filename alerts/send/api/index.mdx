...
1. **[Authentication](./authentication)**: Set up API keys for sending notifications from your backend.
2. **[Send Messages](./send-messages)**: Learn to send notifications using the core `/send` and `/send-batch` endpoints with examples for all channels.
3. **[Push Notifications](./push-notifications)**: Complete guide to mobile push notifications including Firebase setup, sending, and handling in your mobile apps.