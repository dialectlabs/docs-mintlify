...
### New Token Events
New token events trigger when there is a newly-launched token that is outperforming others.

To read more about this event type, see [New Token Events](./events/new-tokens).

## Detecting Interesting Events

Most interesting events are difficult to detect. Some events like "SOL is over $150" may be easy to detect and can sometimes be useful. But it is much more interesting, and much harder, to detect that a token is undergoing unusual price movement, or that one new token is outperforming the many thousands of others that were launched on the same day.
