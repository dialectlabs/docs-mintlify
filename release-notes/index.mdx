...
Enhanced position tracking with new protocol support:

- **Jupiter Borrow Support** - Added Jupiter Borrow protocol support to the Positions API, enabling real-time position tracking for Jupiter's lending positions. [View Positions API docs](/api-reference/positions/list-all-market-positions-by-wallet-address)

## Standard Blinks Library Updates

Infrastructure improvements and API migrations:

- **Jupiter API Migration** - Updated Jupiter swap integration to use the latest Jupiter API endpoints ensuring continued reliability and compatibility. [View Jupiter swap docs](/api-reference/swap/swap-token)
